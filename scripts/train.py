...
def train_sklearn_model(
    ...
    # Low-cardinality categoricals (e.g. canonical body_type, transmission, fuel)
    # are one-hot encoded so trees can split on them cleanly; high-cardinality
    # categoricals (brand, model) are target-encoded. `onehot_columns` selects
    # which cat_columns take the one-hot path; the rest are target-encoded.
    onehot_cols = [c for c in config.get("onehot_columns", []) if c in cat_columns]
    te_cols = [c for c in cat_columns if c not in onehot_cols]
    transformers = [("num", "passthrough", num_columns)]
    if te_cols:
        transformers.append(
            # random_state pins TargetEncoder's internal cross-fit shuffle; without it
            # the encoding (and thus XGB/LightGBM results) is non-reproducible run-to-run.
            ("cat", TargetEncoder(smooth="auto", target_type="continuous", random_state=42), te_cols)
        )
    if onehot_cols:
        transformers.append(
            ("oh", OneHotEncoder(handle_unknown="ignore", sparse_output=False,
                                 min_frequency=10), onehot_cols)
        )

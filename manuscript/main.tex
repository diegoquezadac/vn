...
\node[op, below=0.4cm of x] (ser) {Serialize};
\draw[arr] (x) -- (ser);

\node[dat, below=0.4cm of ser] (s) {$s \in \Sigma^+$};
\draw[arr] (ser) -- (s);

\node[llm, below=0.4cm of s] (ie) 
    {\textbf{IE}\\[-1pt] $\mathrm{LLM}(\mathrm{prompt}_{\mathrm{ext}}, s)$};
\draw[arr] (s) -- (ie);

\node[dat, below=0.4cm of ie] (xhat) 
    {$\hat{\mathbf{x}} = (\hat{a}_1, \dots, \hat{a}_m)$};
\draw[arr] (ie) -- (xhat);

\node[op, below=0.4cm of xhat] (ser2) {Serialize};
\draw[arr] (xhat) -- (ser2);

\node[dat, below=0.4cm of ser2] (shat) {$\hat{s} \in \Sigma^+$};
\draw[arr] (ser2) -- (shat);
...
\node[dat, below=0.4cm of select] (aj)
    {$\tilde{a}_j = c_{i^*} / \hat{a}_j$};
\draw[arr] (select) -- (aj);

\node[outnode, below=0.4cm of aj] (output) 
...
For each open-domain attribute (outer loop, dashed box; stacked blocks 
indicate per-attribute instances), the candidate $\hat{a}_j$ is embedded 
and matched against its catalog $\mathcal{K}_j$ via HNSW, retrieving 
top-$k$ candidates $\mathcal{R}_j$. For each candidate $c_i$ ($\times k$ 
return arrow), the pipeline substitutes $c_i$ into the serialized 
description and evaluates an ER match $M(\hat{s}, \hat{s}_{j \to c_i})$. 
A selection step returns the first match $c_{i^*}$ or falls back to 
$\hat{a}_j$. Closed-domain attributes bypass the loop and feed 
$\hat{a}_j$ directly into the assembly. Novel entities expand the catalog.}
\label{fig:pipeline}
\end{figure}
...
\For{$j \in \mathcal{I}_{\text{open}}$}
    ...
    \For{$c_i \in \mathcal{R}_j$ in ranked order}
        ...
        \If{$M(\hat{s},\, \hat{s}_{j \to c_i}) = 1$}
            \State $\tilde{a}_j \gets c_i$
            \State \textbf{break}
        \EndIf
    \EndFor
    \If{$\tilde{a}_j = \hat{a}_j$}

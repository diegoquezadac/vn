...
\textbf{Serialization.}
For the three tabular datasets, $\text{Serialize}(\mathbf{x})$
concatenates column names with their values
(e.g.\ \textit{col1: val1, col2: val2, \dots}); for free-text listings
the input is used directly. The same operator renders extracted
representations $\hat{\mathbf{x}}$ and candidate replacements
$(\hat{a}_1, \dots, c_i, \dots, \hat{a}_m)$ in the entity-resolution
step.
...
$\text{sim}(\mathbf{e}_j, \mathbf{e}_{c_1}) \geq \tau$ and $\tilde{a}_j
= \hat{a}_j$ otherwise, isolating the gain from the LLM resolver $M$.
...
Our experiments draw on four publicly available automotive listing datasets
...
MuCars~\cite{tabarnoust2024mucars} is a Moroccan dataset with several
...
with fully structured fields. Craigslist~\cite{reese2021craigslist} is a
US dataset characterized by informal free-text descriptions.
...
\hline
\textbf{Dataset} & \textbf{Listings} & \textbf{Brands} & \textbf{Models} & \textbf{Cols} & \textbf{Country} \\
\hline
DVM-CAR     & 268,255 & 101 & 1,011  & 16 & UK \\
MuCars      & 101,896 & 81  & 1,049  & 15 & Morocco \\
AutoScout24 & 251,079 & 47  & 1,312  & 15 & Germany \\
Craigslist  & 426,880 & 42  & 29,667 & 26 & USA \\
\hline
\end{tabular}
\label{tab:datasets}
\end{center}
...
broadest brand coverage in our pool (101 brands). The catalog is not
assumed to be complete: it is expanded at inference time as novel
entities are encountered.

\subsection{Evaluation Data}

\textbf{Record sampling.}
From MuCars, AutoScout24, and Craigslist we draw 200 listings each via
proportional stratified sampling across brand $\times$ fuel type,
yielding 600 evaluation records. Proportional allocation preserves the
population distribution of each attribute in expectation, so
...
whose (brand, model) pair appears in the DVM-CAR catalog, both after
lowercasing and whitespace stripping, in the full population and in
the realized sample. Evaluating model membership at the pair level
(rather than against a flat set of model names) avoids spurious hits
from collisions across brands. Model strings prefixed with the brand
(common in AutoScout24) have the prefix stripped before lookup,
approximating what the IE step extracts at inference time.

\begin{table}[htbp]
\caption{Catalog coverage per dataset (population vs sample, $n = 200$)}
\begin{center}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{|l|r|r|r|r|}
\hline
 & \multicolumn{2}{c|}{\textbf{Brand in $\mathcal{K}$}}
 & \multicolumn{2}{c|}{\textbf{(Brand, Model) pair}} \\
\cline{2-5}
\textbf{Dataset} & \textbf{Pop.} & \textbf{Sample}
                 & \textbf{Pop.} & \textbf{Sample} \\
\hline
AutoScout24 & 98.0\% & 98.0\% & 65.4\% & 65.0\% \\
Craigslist  & 92.8\% & 92.5\% & 24.2\% & 28.5\% \\
MuCars      & 98.9\% & 99.5\% & 63.6\% & 62.5\% \\
\hline
\end{tabular}
\label{tab:sampling}
\end{center}
...
structured form backed by per-attribute data validation. For each record,
...
\textbf{Per-attribute accuracy.}
For each attribute $A_j$ we report the fraction of records for which the
pipeline's normalized value $\tilde{a}_j$ equals the human-annotated
ground-truth value $\tilde{a}_j^*$ (exact match). For the set-valued
\textit{equipment} attribute we report set-level precision, recall, and
F1. Per-attribute accuracies are aggregated into two headline numbers,
open-domain macro-accuracy (brand, model) and closed-domain
macro-accuracy (24 attributes), each reported overall and broken down
by source (AutoScout24, Craigslist, MuCars).

\textbf{Novel-entity handling.}
For brand and model we additionally report precision and recall of the
pipeline's \textit{novel} decision, computed against the human-assigned
novel flags. A false novel (the pipeline marks a record as novel when a
catalog match exists) and a false non-novel (the pipeline matches to a
canonical entity when the ground truth is novel) are both errors, and we
track them separately. This directly evaluates the catalog-expansion
mechanism (Algorithm~\ref{algo:normalization}, lines 16--19).

\textbf{Catalog dynamics.}
We track two quantities that operationalize the ``imperfect catalog,
growing through use'' claim. First, the \textit{catalog growth curve}:
$|\mathcal{K}_j|$ as a function of the number of processed records, per
attribute and per source. Second, \textit{second-pass accuracy}: for the
subset of records in which the pipeline initially produced a novel-entity
decision that was later resolved to a catalog entry (due to catalog
expansion from subsequent records), we re-run the pipeline with the
final catalog and report the change in accuracy. This quantifies the
self-correcting behavior of the design.

\textbf{Error decomposition.}
For every record on which the pipeline disagrees with the ground truth, we
classify the error into one of four categories: (i) extraction error, (ii)
retrieval miss (gold entity not in top-$k$), (iii) resolver error
(false-positive or false-negative match), and (iv) novel-entity
misclassification.

\textbf{Uncertainty.}
For all accuracy metrics we report 95\% bootstrap confidence intervals over
2{,}000 resamples at the record level.

...
$\in \{\text{petrol, diesel, electric, hybrid}\}$) and numerical ranges
...
% Main vertical flow
\node[dat] (x) {$\mathbf{x}$};
\node[dm, right=0.06cm of x] {$1 \times d$};
 
\node[op, below=of x] (ser) {Serialize};
\draw[arr] (x) -- (ser);
 
\node[dat, below=0.14cm of ser] (s) {$s \in \Sigma^+$};
\draw[arr] (ser) -- (s);
 
\node[llm, below=of s] (ie) 
    {\textbf{IE}\\[-1pt] $\mathrm{LLM}(\mathrm{prompt}_{\mathrm{ext}}, s)$};
\draw[arr] (s) -- (ie);
 
\node[dat, below=0.14cm of ie] (xhat) 
    {$\hat{\mathbf{x}} = (\hat{a}_1, \dots, \hat{a}_m)$};
\draw[arr] (ie) -- (xhat);
 
\node[op, below=of xhat] (ser2) {Serialize};
\draw[arr] (xhat) -- (ser2);
 
\node[dat, below=0.14cm of ser2] (shat) {$\hat{s} \in \Sigma^+$};
\draw[arr] (ser2) -- (shat);
 
\node[op, below=of shat] (enc) {Encode};
\draw[arr] (shat) -- (enc);
\node[lb, anchor=west] at ([xshift=0.06cm]enc.east) {$j \in \mathcal{I}_{\mathrm{open}}$};
 
\node[op, below=of enc] (topk) {TopK};
\draw[arr] (enc) -- (topk);
 
\node[dat, below=0.14cm of topk] (rj) 
    {$\mathcal{R}_j = (c_1, \dots, c_k)$};
\draw[arr] (topk) -- (rj);
 
% Substitution + ER
\node[op, below=of rj] (sub) 
    {$\hat{s}_{j \to c_i}$};
\draw[arr] (rj) -- (sub);
 
\node[llm, below=of sub] (er) 
    {\textbf{ER}\\[-1pt] $M(\hat{s}, \hat{s}_{j \to c_i})$};
\draw[arr] (sub) -- (er);
 
% shat feeds ER
\draw[arr, rounded corners=2pt] 
    (shat.west) -- ++(-0.4,0) |- (er.west);
 
% Decision
\node[dat, below=0.14cm of er] (aj) {$a_j$};
\draw[arr] (er) -- (aj);
 
% Match/novel annotation — right of a_j
\node[dm, right=0.06cm of aj] 
    {match: $c_{i^*}$ / novel: $\hat{a}_j$};
 
% Output
\node[outnode, below=of aj] (output) 
    {$\tilde{\mathbf{x}}$};
\draw[arr] (aj) -- (output);
 
% Catalog (right side) — well separated
\node[cat, right=2.0cm of topk] (catalog) 
    {\textbf{Catalog} $\mathcal{K}_j$\\
     $(c_i, \mathbf{e}_i)$\\
     HNSW};
\draw[arr] (catalog) -- (topk)
    node[lb, above, midway] {$\mathrm{sim}(\mathbf{e}_j, \mathbf{e}_i)$};
 
% Encode annotation — aligned with catalog
\node[dm, anchor=west] at (catalog.west |- enc) 
    {$\hat{a}_j \to \mathbf{e}_j \in \mathbb{R}^d$};
 
% Catalog update — route from output.east
\draw[darr, rounded corners=2pt] 
    (output.east) -| (catalog.south)
    node[lb, right, pos=0.65] 
    {update $\mathcal{K}_j$};
 
% Closed-domain bypass
\draw[darr, rounded corners=2pt] 
    (xhat.west) -- ++(-0.5,0) |- (output.west)
    node[lb, left, pos=0.15] 
    {$j \in \mathcal{I}_{\mathrm{closed}}$}
    node[lb, left, pos=0.5] 
    {$a_j = \hat{a}_j$};
 
\end{tikzpicture}%
}
\caption{Vehicle normalization pipeline. The raw record $\mathbf{x}$ is 
serialized and passed to the LLM for attribute extraction (IE). 
Closed-domain attributes (dashed bypass) are assigned directly. 
Open-domain attributes are embedded, matched against a knowledge 
catalog $\mathcal{K}_j$ via HNSW, and resolved (ER) by comparing 
serialized vehicle descriptions. Novel entities expand the catalog.}
\label{fig:pipeline}
\end{figure}


\paragraph{Information extraction.}
Given a serialized description $s$, a single LLM call extracts
...
$a_j = \hat{a}_j$, since the prompt constrains the output to
...
\end{equation}
The iteration proceeds in similarity-ranked order and stops at the
first match; let $i^* = \min\{i : M(\hat{s},\,\hat{s}_{j \to c_i}) = 1\}$
denote the rank of that match. The normalized value is:
\begin{equation}
a_j =
\begin{cases}
c_{i^*} & \text{if a match exists,} \\[4pt]
...
\State \Return $\tilde{\mathbf{x}} = (a_1, \dots, a_m)$
\end{algorithmic}
\end{algorithm}

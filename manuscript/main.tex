...
\textit{brand}, \textit{model}, \textit{submodel}, and \textit{trim level}; the remaining 24 are
...
addresses the three inconsistency axes identified in the introduction: the
...
by informal free-text descriptions. MuCars~\cite{tabarnoust2024mucars} is a
...
\textbf{Rationale.}
Evaluating the extraction component requires labeled pairs
$(s, \hat{\mathbf{x}}^*)$ where $s = \text{Serialize}(\mathbf{x})$ is the
serialized form of a sampled record and
$\hat{\mathbf{x}}^* = (\hat{a}_1^*, \dots, \hat{a}_m^*)$ is its
ground-truth extraction. We obtain records via stratified sampling from
each source and derive $\hat{\mathbf{x}}^*$ from a three-model LLM
ensemble with per-field majority vote.

\textbf{Record sampling.}
We collect 100 records per source (AutoScout24, Craigslist, MuCars)
stratified across brand, fuel type, and transmission type to preserve
distributional heterogeneity, yielding 300 evaluation instances.

\textbf{Annotation protocol.}
Ground truth is produced by a three-model LLM ensemble acting as
independent annotators: \textbf{GPT-5.4} (OpenAI), \textbf{Claude Opus
4.6} (Anthropic), and \textbf{Gemini 3.1 Pro} (Google DeepMind). Each
annotator is prompted zero-shot with the task instruction and the
serialized input. Labels are assigned field-by-field by majority vote;
fields on which all three annotators disagree are marked
\texttt{ABSTAIN} and excluded from evaluation.
...
\textbf{Rationale.}
Evaluating the retrieval component requires labeled triples
$(q, \mathcal{K}_j, c^*)$ where $q$ is an LLM-extracted candidate
$\hat{a}_j$, $\mathcal{K}_j$ is a fixed per-attribute catalog, and
$c^* \in \mathcal{K}_j$ is the correct canonical entity (or $\bot$ if
$q$ refers to a novel entity). Unlike information extraction, IR labels
can be assigned \emph{by construction} when queries are generated from
a known catalog. We adopt this protocol: we fix a catalog, define a
taxonomy of query perturbations motivated by the noise modes observed
in real vehicle listings, and programmatically generate queries whose
gold labels are determined by the generation process itself.
...
dataset~\cite{tabarnoust2025mucars}. MuCars is used as the catalog
source because it was collected from prominent online marketplaces to
ensure market-wide representation and underwent rigorous post-collection
preprocessing, yielding a standardized, curated, and peer-reviewed
reference vocabulary. AutoScout24 and Craigslist are reserved for the
query side of the evaluation.

\textbf{Perturbation taxonomy.}
We identify seven perturbation classes that characterize the gap between
canonical catalog entries and the values a retriever encounters in
practice. The taxonomy is summarized in Table~\ref{tab:pert_taxonomy}.
...
class is populated by mining variants from AutoScout24 and Craigslist
that co-refer to MuCars catalog entries, using two signals: normalized
lexical equality (case, punctuation, whitespace) and row-level
co-occurrence of adjacent attributes (e.g., an AutoScout24 record with
\texttt{brand="VW"} and \texttt{model="Golf"} provides evidence that
\textit{VW} aliases \textit{Volkswagen} in MuCars when MuCars also
contains a \textit{Volkswagen Golf}).
...
case, punctuation, typo, truncation, distractor) and samples alias
queries from the AutoScout24/Craigslist mining step, recording each
as a tuple $(q, A_j, c^*, \text{class})$. Random seeds, edit distances,
and per-class sample sizes are exposed as script parameters to support
reproducibility and ablation. The resulting dataset contains $N$ queries
per attribute, stratified across perturbation classes; exact counts
per attribute and class are reported in Table~\ref{tab:ir_counts} (to
be filled after generation).
...
downstream LLM resolver: any gold entity not retrieved by the IR
component cannot be recovered by the ER component. All metrics are
additionally broken down \emph{per perturbation class}, yielding a
diagnostic profile that isolates which noise modes a given
...
\textbf{Rationale.}
Evaluating the entity resolution component requires labeled triples
$(\hat{s}, \hat{s}_{j \to c_i}, y)$ with $y \in \{0, 1\}$. We synthesize
pairs by applying the perturbation classes of
Table~\ref{tab:pert_taxonomy} to records sampled from the three source
datasets. Because ER is a record-level judgment, we obtain $y$ from the
same three-model LLM ensemble used for information extraction
(Section~\ref{sec:eval_ie}).

\textbf{Record sampling.}
Records are drawn from MuCars, AutoScout24, and Craigslist, stratified
across brand and fuel type to preserve distributional heterogeneity.
We target 100 annotated pairs per source $\times$ per perturbation
class.

\textbf{Dataset generation.}
For each sampled record $\mathbf{x}$ and each open-domain attribute
$A_j$, we construct synthetic pairs by reusing the perturbation classes
of Table~\ref{tab:pert_taxonomy}. Given the extracted value
$\hat{a}_j$, each perturbation class yields a candidate $c_i$ via the
same construction rules used for IR query generation (e.g.\
\textit{case}, \textit{typo}, and \textit{truncation} apply
character-level transformations to $\hat{a}_j$; \textit{alias} draws
from the mined alias table; \textit{distractor} selects a sibling entry
from $\mathcal{K}_j$). Substituting $c_i$ for $\hat{a}_j$ in the
serialized record yields $\hat{s}_{j \to c_i}$, which is paired with
$\hat{s}$ to form the evaluation unit. All constructions are
deterministic given a random seed.

\textbf{Annotation protocol.}
Labels are assigned by the three-model LLM ensemble described in
Section~\ref{sec:eval_ie}. Each annotator receives the two serialized
records $\hat{s}$ and $\hat{s}_{j \to c_i}$ together with the task
instruction and returns $y \in \{0, 1\}$; full prompts are in
...
with a per-class breakdown across the perturbation classes of
Table~\ref{tab:pert_taxonomy}, and Fleiss' $\kappa$~\cite{fleiss1971kappa}
together with the averaged pairwise raw agreement $P_A$ as data-quality
indicators.

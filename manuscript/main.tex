...
fraction of rows whose brand is in $\mathcal{K}_{\text{brand}}$
(resp. whose model is in $\mathcal{K}_{\text{model}}$) after
lowercasing and whitespace stripping, both in the full population and
in the realized sample. Model strings prefixed with the brand (common
in AutoScout24) have the prefix stripped before lookup, approximating
what the IE step extracts at inference time.
...
\hline
 ...
 & \multicolumn{2}{c|}{\textbf{Model in $\mathcal{K}$}} \\
\cline{2-5}
\textbf{Dataset} & \textbf{Pop.} & \textbf{Sample}
                 & \textbf{Pop.} & \textbf{Sample} \\
\hline
AutoScout24 & 98.0\% & 98.0\% & 71.2\% & 70.0\% \\
Craigslist  & 92.8\% & 92.5\% & 25.0\% & 31.0\% \\
MuCars      & 98.9\% & 99.5\% & 64.9\% & 64.0\% \\
\hline
\end{tabular}
\label{tab:sampling}
\end{center}

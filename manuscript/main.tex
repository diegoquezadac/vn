...
Table~\ref{tab:schema} lists the $m = 19$ attributes of the schema
used in this study, and Table~\ref{tab:catalog} specifies the domain
$\mathcal{V}_j^{\text{closed}}$ of each closed-domain categorical
attribute. The decomposition is designed to resolve the three axes
of inconsistency identified in the introduction: the
separation of \textit{energy source}, \textit{propulsion system},
and \textit{fuel type} into distinct attributes addresses scope
inconsistency, while the separation of \textit{transmission type},
\textit{technology}, and \textit{gears} addresses granularity
inconsistency. The schema is not fixed by the framework; practitioners
may define alternative schemas to suit their application.

\begin{table}[h]
\caption{Proposed Normalized Schema}
\label{tab:schema}
\centering
\footnotesize
\begin{tabular}{ll}
...
Engine size & Total volume of all cylinders \\
...
\begin{table}[h]
\caption{Domain of Closed-Domain Attributes}
\label{tab:catalog}
\centering
\footnotesize
...
\hline
Propulsion system & icev, ev, pev, bev, hev, mhv, phev, erev, fcev, pfcev, other, unknown \\
\hline
...
reflect raw seller input rather than a controlled vocabulary. MuCars is a
...
\textbf{Evaluation setup.} For AutoScout24 and MuCars, which contain no
free-text field, the input is the serialized structured row (comma-separated
\textit{col:\,value} pairs). For Craigslist, the raw free-text listing

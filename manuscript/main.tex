...
\hat{a}_m^*)$ is the correct extraction for vehicle $s$. We use a
three-model LLM ensemble as proxy annotators, resolving per-field
disagreements by majority vote.
...
the input example. Labels are assigned field-by-field by majority vote (at least two of three annotators
...
construction} when queries are generated from a known catalog. We adopt
...
Algorithm~\ref{algo:normalization}.
